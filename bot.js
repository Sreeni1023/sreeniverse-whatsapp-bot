    ...
    try {
        ...
        sock.ev.on('messages.upsert', async ({ messages, type }) => {
            try {
                ...
                // CHECK handler for eligibility
                if (messageText.trim().toUpperCase().startsWith("CHECK")) {
                    try {
                        const axios = require("axios");
                        const response = await axios.post("http://127.0.0.1:5678/webhook/eligibility-check", {
                            phone: phone,
                            message: messageText.trim()
                        }, { timeout: 10000 });
                        const decision = response.data?.decision || response.data?.Decision || JSON.stringify(response.data);
                        await sock.sendMessage(from, { text: decision });
                    } catch (err) {
                        await sock.sendMessage(from, { text: "❌ Eligibility check failed. Please try again." });
                    }
                    return;
                }
                let replyText = null;
